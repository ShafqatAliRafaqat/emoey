...
Class Facebook
{
     ...
     public function connect_url()
    {
        ...
        $uri = $this->helper->getLoginUrl(
            base_url() . $this->config->item('facebook_connect_redirect_url'),
            $this->config->item('facebook_permissions')
        );
    ...
    {
        $this->requestAccesstokenRepeat();

    }
    private function requestAccesstokenRepeat()
    {
        $access_token = null;
        if (!$access_token)
        {
