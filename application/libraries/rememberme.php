...
class Rememberme {
	...
	function setCookie($netid = "", $nocookie = false) {
		...
		$this->CI->db->delete('bc_ci_cookies');
		...
		if ($nocookie) {
			...
			$query = $this->CI->db->get_where('bc_ci_cookies', array(
				'php_session_id' => session_id()
			));
			if ($query->num_rows()) {
				$orig_page_requested = $query->row()->orig_page_requested;
			}
			$this->CI->db->delete('bc_ci_cookies', array(
				'php_session_id' => session_id()
			));
		}
		...
		$this->CI->db->insert('bc_ci_cookies', $insertdata);
		...
		if (!$nocookie) {
			...
			// establish session
			$this->CI->session->set_userdata('bc_session', $netid);	
		}		
	}
	...
	function verifyCookie() {	
		...
		//echo str_replace('.', '_', $_SERVER['SERVER_NAME']).'@@@'.$this->CI->input->cookie('rmtoken_' . str_replace('.', '_', $_SERVER['SERVER_NAME']));
		...
		//print $this->CI->db->last_query();die;
